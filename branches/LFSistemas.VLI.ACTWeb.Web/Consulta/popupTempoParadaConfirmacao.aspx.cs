...
{
    public partial class popupTempoParadaConfirmacao : System.Web.UI.Page
    {
        #region [ PROPRIEDADES ]
        ...
        TempoParadaConfirmacaoController tempoParadaConfirmacaoController = new TempoParadaConfirmacaoController();
        ...
        //    DateTime dataIni = txtDataInicial.Text.Length > 0 && txtHoraInicial.Text.Length > 0 ? DateTime.Parse(txtDataInicial.Text + " " + FormataHora(txtHoraInicial.Text, "txtHoraInicial.Text")) : DateTime.Now;
        //    DateTime dataFim = txtDataFinal.Text.Length > 0 && txtHoraFinal.Text.Length > 0 ? DateTime.Parse(txtDataFinal.Text + " " + FormataHora(txtHoraFinal.Text, "txtHoraFinal.Text")) : DateTime.Parse(string.Format("{0} 23:59:59", DateTime.Now.AddDays(-1)));
        ...
        //    sb.AppendLine("ESTAÇÃO;DESCRIÇÃO;DATA INICIAL;DATA FINAL;TTR");

        //    foreach (var alarmes in itens)
        //    {
        //        sb.AppendLine(string.Format("{0};{1};{2};{3};{4}", alarmes.Estacao, alarmes.Descricao, alarmes.DataInicial, alarmes.DataFinal, alarmes.TTR));
        //    }

        //    Response.Clear();
        //    Response.ContentEncoding = Encoding.GetEncoding("iso-8859-1");
        //    Response.AddHeader("content-disposition", "attachment; filename=Relatorio_AlarmesTelecomandadas.csv");
        ...
        protected void Pesquisar(string ordenacao, Navigation navigation)
        {
            ...
            var dataIni = txtDataInicial.Text.Length > 0 && txtHoraInicial.Text.Length > 0 ? txtDataInicial.Text + " " + FormataHora(txtHoraInicial.Text, "Inicial") : DateTime.Now.ToShortDateString();
            var dataFim = txtDataFinal.Text.Length > 0 && txtHoraFinal.Text.Length > 0 ? txtDataFinal.Text + " " + FormataHora(txtHoraFinal.Text, "Final") : string.Format("{0} 23:59:59", DateTime.Now.AddDays(-1));
            

            var itens = tempoParadaConfirmacaoController.ObterTempoParadaConfirmacao(new TempoParadaConfirmacao()
            {
                //Estacao = null,
                DataInicial = dataIni.ToString(),
                DataFinal = dataFim.ToString(),

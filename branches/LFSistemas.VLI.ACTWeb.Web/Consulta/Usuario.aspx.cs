...
namespace LFSistemas.VLI.ACTWeb.Web.Consulta
{
    public partial class Usuario : System.Web.UI.Page
    {
        ...
        public List<Usuarios> itens { get; set; }
        ...
        {
            Response.Redirect("/Cadastro/Usuario.aspx?matricula=" + Uteis.Criptografar("novo", "a#3G6**@") + "&flag=novousuario&lu=" + Uteis.Criptografar(ViewState["ulNome"].ToString().ToLower(), "a#3G6**@") + "&mu=" + Uteis.Criptografar(ViewState["uMatricula"].ToString().ToLower(), "a#3G6**@") + "&pu=" + Uteis.Criptografar(ViewState["uPerfil"].ToString().ToLower(), "a#3G6**@") + "&mm=" + Uteis.Criptografar(ViewState["ulMaleta"].ToString().ToLower(), "a#3G6**@"));
        }
        protected void lnkUsuarios_Click(object sender, EventArgs e)
        {
            LinkButton btn = (LinkButton)(sender);
            string matricula = btn.CommandArgument;
            Response.Redirect("/Cadastro/Usuario.aspx?matricula=" + Uteis.Criptografar(matricula.ToLower(), "a#3G6**@") + "&flag=consulta&lu=" + Uteis.Criptografar(ViewState["ulNome"].ToString().ToLower(), "a#3G6**@") + "&mu=" + Uteis.Criptografar(ViewState["uMatricula"].ToString().ToLower(), "a#3G6**@") + "&pu=" + Uteis.Criptografar(ViewState["uPerfil"].ToString().ToLower(), "a#3G6**@") + "&mm=" + Uteis.Criptografar(ViewState["ulMaleta"].ToString().ToLower(), "a#3G6**@"));
        }
        ...
        protected void Pesquisar(string ordenacao, Navigation navigation)
        {
            ...
            itens = usuarioController.ObterTodos(new Usuarios()
            {
            ...
            if (itens.Count > 0)
            {
                switch (ordenacao)
                {
                    ...
                    case "MATRICULA DESC":
                        ...
                        break;
                    case "PERFIL ASC":
                        itens = itens.OrderBy(o => o.Perfil_Abreviado).ToList();
                        break;
                    case "PERFIL DESC":
                        itens = itens.OrderByDescending(o => o.Perfil_Abreviado).ToList();
                        break;
                    case "EMAIL ASC":
                        itens = itens.OrderBy(o => o.Email).ToList();
                        break;
                    case "EMAIL DESC":
                        itens = itens.OrderByDescending(o => o.Email).ToList();
                        break;
                    case "MALETA ASC":
                        itens = itens.OrderBy(o => o.CodigoMaleta).ToList();
                        break;
                    case "MALETA DESC":
                        itens = itens.OrderByDescending(o => o.CodigoMaleta).ToList();
                        break;
                    case "ULT. ACESSO ASC":
                        itens = itens.OrderBy(o => o.Ultimo_Acesso).ToList();
                        break;
                    case "ULT. ACESSO DESC":
                        itens = itens.OrderByDescending(o => o.Ultimo_Acesso).ToList();
                        break;
                    case "ATIVO ASC":

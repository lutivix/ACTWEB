<%@ Page Title="" Language="C#" MasterPageFile="~/ACTWEB.Master" AutoEventWireup="true" CodeBehind="Consultar_Banners.aspx.cs" Inherits="LFSistemas.VLI.ACTWeb.Web.DadosApoio.Consultar_Banners" %>
            ...
            <table style="width: 100%; margin: 10px;">
                <tr>
                    <td style="width: 70%; padding-top: 10px;">
                        <label for="matricula">Atquivo:</label>
                    ...
                    <td style="width: 30%; padding-top: 10px;">
                        ...
                        <asp:TextBox runat="server" ID="txtFiltroDescricao" CssClass="form-control" Width="98%" />
                    </td>
                </tr>
                <tr>
                        ...
                        <tr>
                            <td>
                                <asp:Repeater ID="RepeaterItens" runat="server">
                                            ...
                                            <thead>
                                                <tr>
                                                    <th style="width: 46%; text-align: center; font-size: 12pt; background-color: rgb(55, 119, 188); border-right: 1px solid rgb(0, 72, 89);">
                                                        <asp:LinkButton runat="server" ID="lnkArquivo" OnClick="lnkArquivo_Click" Text="Arquivo" ForeColor="White" />
                                                    </th>
                                                    <th style="width: 46%; text-align: center; font-size: 12pt; background-color: rgb(55, 119, 188); border-right: 1px solid rgb(0, 72, 89);">
                                    ...
                                    <ItemTemplate>
                                        <tr style="background-color: #eee; cursor: pointer;" onclick="javascript:window.location='/DadosApoio/Manter/Banners.aspx?di=<%# LFSistemas.VLI.ACTWeb.Entities.Uteis.Criptografar(Eval("Banner_ID").ToString(), "a#3G6**@") %>'">
                                            <td style="width: 46%; text-align: left; border-right: 1px solid rgb(0, 72, 89);" title="<%# Eval("Arquivo")%>"><%# Eval("Arquivo")%> </td>
                                            <td style="width: 46%; text-align: left; border-right: 1px solid rgb(0, 72, 89);" title="<%# Eval("Descricao")%>"><%# Eval("Descricao")%> </td>
                                            <td class="ativo-<%# Eval("Ativo").ToString().Substring(0, 1) %>" style="width: 06%; text-align: center;" title="<%# Eval("Ativo")%>"><%# Eval("Ativo")%> </td>
                                        </tr>
                                    </ItemTemplate>
                                    <AlternatingItemTemplate>
                                        <tr style="background-color: #fff; cursor: pointer;" onclick="javascript:window.location='/DadosApoio/Manter/Banners.aspx?di=<%# LFSistemas.VLI.ACTWeb.Entities.Uteis.Criptografar(Eval("Banner_ID").ToString(), "a#3G6**@") %>'">
                                            <td style="width: 46%; text-align: left; border-right: 1px solid rgb(0, 72, 89);" title="<%# Eval("Arquivo")%>"><%# Eval("Arquivo")%> </td>
                                            <td style="width: 46%; text-align: left; border-right: 1px solid rgb(0, 72, 89);" title="<%# Eval("Descricao")%>"><%# Eval("Descricao")%> </td>
                                            <td class="ativo-<%# Eval("Ativo").ToString().Substring(0, 1) %>" style="width: 06%; text-align: center;" title="<%# Eval("Ativo")%>"><%# Eval("Ativo")%> </td>
                                        </tr>

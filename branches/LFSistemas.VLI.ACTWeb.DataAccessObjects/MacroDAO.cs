            ...
            try
            {
                using (var connection = ServiceLocator.ObterConexaoACTWEB())
                {
                    ...
                    if (origem == "tela_consulta")
                    {
                        ...
                        query.Append(@"SELECT 'R' AS R_E, MR.MR_GRMN AS ID, MR.MR_MSG_TIME AS Horário, MC.MCT_NOM_MCT AS Loco, MR.MR_MC_NUM AS Macro, MR.MR_TEXT AS Mensagem, SUBSTR(MR.MR_TEXT, 1, 760) AS Texto, MR.MR_MCT_ADDR AS MCT, MR.MR_PRF_ACT AS Trem, MR.MR_COD_OF AS CodOS, PF.MFP_LEITURA AS Leitura, PF.MPF_ID AS Leitura_ID, MR.MR_CORREDOR 
                    ...
                    {
                        query.Append(@"SELECT 'R' AS R_E, MR.MR_GRMN AS ID, MR.MR_MSG_TIME AS Horário, MC.MCT_NOM_MCT AS Loco, MR.MR_MC_NUM AS Macro, MR.MR_TEXT AS Mensagem, SUBSTR(MR.MR_TEXT, 1, 760) AS Texto, MR.MR_MCT_ADDR AS MCT, MR.MR_PRF_ACT AS Trem, MR.MR_COD_OF AS CodOS, PF.MFP_LEITURA AS Leitura, PF.MPF_ID AS Leitura_ID, US.NOME, MR.MR_MSG_LIDA, MR.MR_MSG_RESP, MR.MR_CORREDOR
                                        FROM ACTPP.MENSAGENS_RECEBIDAS MR, ACTPP.MCTS MC, ACTPP.MSG_PF PF, ACTWEB.USUARIOS US 
                                            WHERE MC.MCT_ID_MCT = MR.MR_MCT_ADDR 
                                              AND PF.MFP_ID_MSG = MR.MR_GRMN
                                              ...
                                              AND SUBSTR(MR.MR_TEXT,2,4) = '7000' 
                                        UNION                                         
                                        SELECT 'E' AS R_E, ME.ME_GFMN AS ID, ME.ME_MSG_TIME AS Horário, MC.MCT_NOM_MCT AS Loco, ME.ME_MAC_NUM AS Macro, ME.ME_TEXT AS Mensagem, SUBSTR(ME.ME_TEXT, 1, 760) AS Texto, ME.ME_MCT_ADDR AS MCT, ME.ME_PRF_ACT AS Trem, ME.ME_COD_OF AS CodOS, 'T' AS Leitura, 0 AS Leitura_ID, US.NOME, ME.ME_MSG_LIDA, ME.ME_MSG_RESP, ME.ME_CORREDOR 
                                        FROM ACTPP.MENSAGENS_ENVIADAS ME, ACTPP.MCTS MC, ACTWEB.USUARIOS US
                                            WHERE MC.MCT_ID_MCT = ME.ME_MCT_ADDR 
                    ...
                    if (!string.IsNullOrEmpty(filtro.Corredores))
                        query.Replace("${CORREDOR}", string.Format("AND (MR.MR_CORREDOR IN ({0}) OR MR.MR_CORREDOR IS NULL)", filtro.Corredores));
                    else
            ...
            try
            {
                using (var connection = ServiceLocator.ObterConexaoACTWEB())
                {
                    ...
                    if (!string.IsNullOrEmpty(filtro.NumeroLocomotiva))
                        query.Replace("${R_Locom}", string.Format("AND UPPER(MCTS.MCT_NOM_MCT) IN ({0})", filtro.NumeroLocomotiva.ToUpper()));
                    else
                    ...
                    if (!string.IsNullOrEmpty(filtro.NumeroMacro))
                        query.Replace("${R_Macro}", string.Format("AND MENSAGENS_RECEBIDAS.MR_MC_NUM IN ({0})", filtro.NumeroMacro));
                    else
                    ...
                    if (!string.IsNullOrEmpty(filtro.NumeroLocomotiva))
                        query.Replace("${E_Locom}", string.Format("AND MCTS.MCT_NOM_MCT IN ({0})", filtro.NumeroLocomotiva));
                    else
        ...
        public List<Macro> ObterEnviadas(FiltroMacro filtro)
        {
            ...
            try
            {
                using (var connection = ServiceLocator.ObterConexaoACTWEB())
                {
                    ...
                    {
                        queryE.Replace("${DataInicio}", string.Format("to_date('{0}','DD/MM/YYYY HH24:MI:SS')", filtro.DataInicio));
                    }
                    else
                        queryE.Replace("${DataInicio}", "");

                    if (filtro.DataFim != null)
                    {
                        queryE.Replace("${DataFinal}", string.Format("to_date('{0}','DD/MM/YYYY HH24:MI:SS')", filtro.DataFim));
                    }
                    else
                        queryE.Replace("${DataFinal}", "");


                    if (!string.IsNullOrEmpty(filtro.NumeroLocomotiva))
                        queryE.Replace("${ME_LOCO}", string.Format("AND MCTS.MCT_NOM_MCT IN ({0})", filtro.NumeroLocomotiva));
                    else
                    ...
                    if (!string.IsNullOrEmpty(filtro.NumeroTrem))
                    {
                        ...
                        var clausula = string.Join("' or ME_PRF_ACT like '", trem);
                        clausula = string.Concat("ME_PRF_ACT like '", clausula, "'");
                    ...
                    if (!string.IsNullOrEmpty(filtro.NumeroMacro))
                    {
                        queryE.Replace("${ME_MAC_NUM}", string.Format("AND ME_MAC_NUM IN ({0})", filtro.NumeroMacro));
                    }
                    else
                    {
                    ...
                    if (!string.IsNullOrEmpty(filtro.Expressao))
                    {
                        ...
                        for (int i = 0; i < espressao.Length; i++)
                            espressao[i] = "%" + espressao[i].ToUpper() + "%";
                        var clausula = string.Join("' or ME_TEXT like '", espressao);
                        clausula = string.Concat("ME_TEXT like '", clausula, "'");
                        ...
                        if (verificastring.Read())
                        {
                            using (var comando = connection.CreateCommand())
                            {
                                ...
                                else
                                {
                                    if (tipo == "R")
                                    {
                                        comando.CommandText = @"select mr.MR_GRMN, mr.MR_LOCO, mr.MR_PRF_ACT, mr.MR_COD_OF, mr.MR_MSG_TIME, mr.MR_MC_NUM, mr.MR_TEXT, mr.MR_MCT_ADDR, eo.ES_ID_EFE, ed.ES_ID_EFE,t.TM_NUM_VAG, t.TM_TON_BRT,m.MCT_OBC_VERSAO,m.MCT_MAP_VERSAO,t.TM_CMP_TR, MR_NOME_SB, MR_KM
                                from ACTPP.mensagens_recebidas mr, ACTPP.estacoes eo, ACTPP.estacoes ed, ACTPP.trens t, ACTPP.mcts m

...
namespace LFSistemas.VLI.ACTWeb.DataAccessObjects
{
    class RelatoriosAlarmesDAO
    {
        ...
        public List<Corredores> filtroCorredor()
        {
            ...
            try
            {
                using (var connection = ServiceLocator.ObterConexaoACTWEB())
                {
                    ...
                    query.Append(@"SELECT NM_COR_ID, NM_COR_NOME
                                   FROM NOME_CORREDOR
                                   WHERE NM_COR_ATIVO_SN = 'S'");
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            Corredores item = preencherCorredor(reader);
                            itens.Add(item);
                        }
                    }
        ...
        public List<Estacao> filtroEstacoes()
        {

            #region [ PROPRIEDADES ]

            StringBuilder query = new StringBuilder();
            List<Estacao> itens = new List<Estacao>();
            ...
            try
            {
                using (var connection = ServiceLocator.ObterConexaoACTWEB())
                {
                    ...
                    query.Append(@"SELECT ES_ID_NUM_EFE, ES_ID_EFE, ES_DSC_EFE FROM ESTACOES");
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            Estacao item = preencherEstacao(reader);
                            itens.Add(item);
                        }
                    }
        ...
        public List<Estacao> filtroTipoAlarme()
        {
            ...
            try
            {
                using (var connection = ServiceLocator.ObterConexaoACTWEB())
                {
                    ...
                    query.Append(@"SELECT * FROM TIPOS_ALARMES;");
                    ...
                    using (var reader = command.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            Estacao item = preencherTipoAlarme(reader);
                            itens.Add(item);
                        }
                    }

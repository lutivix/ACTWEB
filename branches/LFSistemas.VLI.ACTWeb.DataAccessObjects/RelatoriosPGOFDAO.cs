        ...
        public List<LocomotivasCorredor> ObterLocomotivasPorCorredor(string Localidades)
        {
            ...
            try
            {
                using (var connection = ServiceLocator.ObterConexaoPGOF())
                {
                    ...
                    {
                        query.Append(@"SELECT UNIDADE_PRODUCAO, LOCAL_FERROVIARIO, PREFIXO, TREM, DATA_FORMACAO, PREVISAO_CHEGADA_DESTINO, CAPACIDADE_TRACAO_TOTAL, QTD_LOCOMOTIVAS, QTD_VAGOES_VAZIOS, QTD_VAGOES_CARREGADOS, TU, TB, NUM_LOCOMOTIVAS, LOCOMOTIVAS,SIGLA_MODELO_LOCOMOTIVA, DESCRICAO_MODELO_LOCOMOTIVA, CODIGO_SITUACAO, HORAS_FORA_OFICINA, DATA_SAIDA_OFICINA, DATA_HORA_PREVISAO_MANUTENCAO, GIRO_LOCOMOTIVA, DSC_PRODUTO FROM (" + ServiceLocator.ObterQueryLocoTrem() + ") ORDER BY LOCAL_FERROVIARIO");
                    }
                    else
                    {
                        query.Append(@"SELECT UNIDADE_PRODUCAO, LOCAL_FERROVIARIO, PREFIXO, TREM, DATA_FORMACAO, PREVISAO_CHEGADA_DESTINO, CAPACIDADE_TRACAO_TOTAL, QTD_LOCOMOTIVAS, QTD_VAGOES_VAZIOS, QTD_VAGOES_CARREGADOS, TU, TB, NUM_LOCOMOTIVAS, LOCOMOTIVAS,SIGLA_MODELO_LOCOMOTIVA, DESCRICAO_MODELO_LOCOMOTIVA, CODIGO_SITUACAO, HORAS_FORA_OFICINA, DATA_SAIDA_OFICINA, DATA_HORA_PREVISAO_MANUTENCAO, GIRO_LOCOMOTIVA, DSC_PRODUTO FROM (" + ServiceLocator.ObterQueryLocoTrem() + ") WHERE LOCAL_FERROVIARIO IN (" + Localidades + ") ORDER BY LOCAL_FERROVIARIO");
                    }
        ...
        public List<LocomotivasCorredor> ObterTrensParadosLicenciados(string Localidades)
        {
            ...
            try
            {
                using (var connection = ServiceLocator.ObterConexaoPGOF())
                {
                    ...
                    /*ESTAÇÕES VS LOCAIS(SBS)
                    SELECT EVE.*,E.ES_DSC_EFE, E.ES_ID_eFE, EV.EV_NOM_MAC 
                    FROM ELEM_VIA EV, ELEM_VIA_ESTACOES EVE, ESTACOES E 
                    WHERE EV.EV_ID_ELM = EVE.EV_ID_eLM AND E.ES_ID_NUM_EFE = EVE.ES_ID_nUM_EFE
                    AND EV.EV_IND_TP_SECAO IN ('1','2','P','D')
                    ORDER BY ES_ID_EFE;
                    TRENS PARADOS
                    SELECT 'UNILOG' "SISTEMA",T.TM_ID_TRM "TREM",UTP.* FROM UNL_TRENS_PARADOS UTP, TRENS T 
                    WHERE T.ST_ID_SIT_TREM = 4 AND UTP.ID_TREM_ACT = T.TM_ID_TRM AND UTP.DT_FIM_PARADA IS NULL  AND TM_COD_OF IS NOT NULL AND TM_COD_OF <> 0 */

                    /*SELECT 'UNILOG' "SISTEMA",T.TM_ID_TRM "TREM",T.TM_COD_OF "OS", T.TM_PRF_ACT "PREFIXO", UTP.* FROM UNL_TRENS_PARADOS UTP, TRENS T 
                    WHERE T.ST_ID_SIT_TREM = 4 AND UTP.ID_TREM_ACT = T.TM_ID_TRM AND UTP.DT_FIM_PARADA IS NULL  AND TM_COD_OF IS NOT NULL AND TM_COD_OF <> 0
                    LICENCIADOS
                    select distinct(tm_id_trm) from licencas where li_st_lic in ('E','C'); */
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        private LocomotivasCorredor PreencherPropriedadesLocomotivasPorTempoAcumulado(OleDbDataReader reader)
        {
            ...
            {  // UNIDADE_PRODUCAO, LOCAL_FERROVIARIO, PREFIXO, TREM, DATA_FORMACAO, PREVISAO_CHEGADA_DESTINO, CAPACIDADE_TRACAO_TOTAL, QTD_LOCOMOTIVAS, 
                ...
                //if (!reader.IsDBNull(16)) item.Estado = reader.GetString(16);
        ...
        private LocomotivasCorredor PreencherPropriedadesLocomotivasPorConfiabilidade(OleDbDataReader reader)
        {
            ...
            {  // UNIDADE_PRODUCAO, LOCAL_FERROVIARIO, PREFIXO, TREM, DATA_FORMACAO, PREVISAO_CHEGADA_DESTINO, CAPACIDADE_TRACAO_TOTAL, QTD_LOCOMOTIVAS, 
                ...
                if (!reader.IsDBNull(18)) item.UltDataManut = reader.GetString(18);
                if (!reader.IsDBNull(17)) item.TempoForaOficina = reader.GetValue(17).ToString();
                if (!reader.IsDBNull(19)) item.TempoVoltarOficina = reader.GetString(19);
        ...
        private LocomotivasCorredor PreencherPropriedadesLocomotivasPorProduto(OleDbDataReader reader)
        {
            ...
            {  // UNIDADE_PRODUCAO, LOCAL_FERROVIARIO, PREFIXO, TREM, DATA_FORMACAO, PREVISAO_CHEGADA_DESTINO, CAPACIDADE_TRACAO_TOTAL, QTD_LOCOMOTIVAS, 
                ...
                if (!reader.IsDBNull(13)) item.Locomotivas = reader.GetValue(13).ToString();
                if (!reader.IsDBNull(21)) item.Produto = reader.GetString(21);
